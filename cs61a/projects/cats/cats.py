...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    min_num = min(diff_list, key = abs)
    if(abs(min_num) > limit):
        return typed_word
    min_index = [i for i in range(len(diff_list)) if min_num == diff_list[i]]
    for x in min_index:
        if(typed_word == word_list[x]):
            return word_list[x]
    return word_list[min_index[0]]
...
def furry_fixes(typed, source, limit):
    ...
    if(typed[0] == source[0]):
        count = furry_fixes(typed[1:], source[1:], limit)
    else:
        count = 1 + furry_fixes(typed[1:], source[1:], limit - 1)
    return count
...
def minimum_mewtations(typed, source, limit):
    ...
    if limit < 0:
        return 1# Base cases should go here, you may add more base cases as needed.
        # BEGIN
        "*** YOUR CODE HERE ***"
    ...
    if (typed[0] == source[0]): # Feel free to remove or add additional cases
        # BEGIN
        "*** YOUR CODE HERE ***"
        count = 0 + minimum_mewtations(typed[1:], source[1:],limit)
        # END
    else:
        # f(n - 1)+ add / remove / substitute = f(n)
...
def final_diff(typed, source, limit):
    ...
    If you implement this function, it will be used.""" "Remove this line to use your final_diff function."
    if limit < 0:
        return 1# Base cases should go here, you may add more base cases as needed.
        # BEGIN
        "*** YOUR CODE HERE ***"
...
def report_progress(typed, source, user_id, upload):
    ...
    #source_words = split(source)
    count = 0
    while(count < min(len(typed), len(source)) and typed[count] == source[count]):
        count += 1
    progress = count / len(source)
    user_upload = {'id': user_id, 'progress': progress}
    upload(user_upload)
    return progress
    # END PROBLEM 8
...
def fastest_words(words_and_times):
    ...
    # BEGIN PROBLEM 10
    "*** YOUR CODE HERE ***"
    # 创建player个数的嵌套列表，每个列表的元素为当pw用时与最小用时相同时，times索引所对应的单词
    fw = [[] for x in player_indices]
    for x in word_indices:
        for y in player_indices:
            if (get_time(times, y, x) == min([get_time(times, z, x) for z in player_indices])):
                fw[y] += [words[x]]
                break
