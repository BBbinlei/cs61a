...
class FreeChecking(Account):
    ...
    "*** YOUR CODE HERE ***"
    def __init__(self, account_holder):
        self.free_number = self.free_withdrawals
        super().__init__(account_holder)
    def withdraw(self, amount):
        if self.free_number > 0:
            self.free_number -= 1
...
def without(s, i):
    ...
    s = s.rest
    while(s != Link.empty):
        c.rest = Link(s.first, s.rest)
        c = c.rest
        s = s.rest

    before = Link.empty
    now = copy
    while(now != Link.empty):
        if(i == 0):
            if before != Link.empty:
                before.rest = now.rest
                return copy
            else:
                return copy.rest
        else:
